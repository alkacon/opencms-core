...
$.widget("ui.sortable", $.extend({}, $.ui.mouse, {
	...
	_rearrange: function(event, i, a, hardRefresh) {
		...
		if(!this.options.nested || !$.ui.isOverAxis(this.positionAbs.left, i.left, i.width)){
			a ? a[0].appendChild(this.placeholder[0]) : i.item[0].parentNode.insertBefore(this.placeholder[0], (this.direction == 'down' ? i.item[0] : i.item[0].nextSibling));
		}else{
			
			if(i.item.find(this.options.nested).siblings().length<=0){
				i.item.append("<ul></ul>");
				// check if nested is a tag
					// else create a div with the class
					
					//console.log("adding");
			}
			
			i.item.find(this.options.nested)[0].appendChild(this.placeholder[0]);
	...
	_hasChildNodes:function(item){
		if(this.options.nested){
			if(item.find(this.options.nested) && item.find(this.options.nested).children(this.options.item).length>0){
				return true;	
			}else{
				return false;	
			}
