...
/* Initializes the tablegallery popup window. */
function initPopup() {
...
function okPressed() {
	...
	if ( initValues.fieldid != null && initValues.fieldid != "") {
		//write table to the input field of the widget
		window.opener.document.getElementById(initValues.fieldid).value  = activeItem.htmltable;
		// copy value to div in order to display it 
		window.opener.checkTableContent(initValues.fieldid);
	}
	window.close();
}