# Manual Storage Schema Update for HSQLDB
...
```sql
CREATE CACHED TABLE CMS_STORAGE (
...
CREATE INDEX CMS_CONTENTS_05_IDX ON CMS_CONTENTS (HASH, STORAGE);
CREATE INDEX CMS_CONTENTS_07_IDX ON CMS_CONTENTS (STORAGE, HASH);
...
CREATE INDEX CMS_OFFLINE_CONTENTS_01_IDX ON CMS_OFFLINE_CONTENTS (HASH, STORAGE);
CREATE INDEX CMS_OFFLINE_CONTENTS_02_IDX ON CMS_OFFLINE_CONTENTS (STORAGE, HASH);
```
