...
\item project Directories: /release/
...
The installation page is displayed with the new link to your page ``help.html". Click on the link to
view your new page (Figure~\ref{demopage03}) without any content. 
Of course you can enter additional text with the html editor as desribed above.
...
\item Edit sourcecode: Clicking opens a text editor in which the file can be modified.
...
\item Change navigation: Here the navigation text and position of a resource can be changed.

\item Properties: To every resource properties can be attached as key / value pairs, e.g. the title or the description is stored as a resource property.
Properties can be added, modified or deleted.

\item Standard functions such as ``Copy,"
``Rename", ``Move" and ``Delete" can be selected from the context-sensitive
menus. ``Touch" changes the timestamp of a resource, which marks it as changed, 
and it will be published when the project is published.

\item Undelete resources: \index{undelete} When a resource
...
\item Restore a version from history  \index{history}
...
\item Publish a resource \index{publish resource}
