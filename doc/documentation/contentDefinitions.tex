...
data with an appropriate Back Office module (see chapter \ref{Backoffice Modules}
...
The Content Definition should implement the {\name I\_CmsContent interface}.
This interface is only a marker interface, so no methods have to be
implemented.
...
the OpenCms Backoffice (from the OpenCms workplace) a constructor has to 
exist that takes the \\
{\class CmsObject} and an Integer object as parameters.
...
\jtaba  \xspace */\\
\jtaba public NewsContentDefinition(CmsObject cms, int id) \{\\
\jtabb  // read data from data source and\\
\jtabb  // initialize the member variables here\\
\jtaba \}\\
\end{java}

Creating a new entry is done with a constructor as well. This time, the
constructor gets all the data of a news entry and creates a new Content Definition
...
already implemented. This class has the member fields to store group, owner and
access flags for an entry and also methods to check the access rights for a user.
To check if an entry can be read or write the methods
{\meth isReadable()} and {\meth isWriteable()} are used. This is the default implementation
in the abstract class {\class A\_CmsContentDefinition} of these methods:
