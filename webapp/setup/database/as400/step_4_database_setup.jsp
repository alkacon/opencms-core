...
<table border="0" cellpadding="2" cellspacing="0">
	...
	<tr>
		<td>&nbsp;</td>
		<td colspan="2"><input type="checkbox" name="createTables" value="true" checked> Create tables 
		</td>
...
Enter the JDBC <b>Connection String</b> to your database (without libraries parameter, ie: <code>jdbc:as400://xxx.xxx.xxx.xxx</code>).
<%= Bean.getHtmlPart("C_HELP_END") %>

<%= Bean.getHtmlPart("C_HELP_START", "4") %>
Enter the name of the AS400 <b>Library</b> which should be used by Alkacon OpenCms.
<%= Bean.getHtmlPart("C_HELP_END") %>

<%= Bean.getHtmlPart("C_HELP_START", "5") %>
The setup wizard <b>creates</b> only the tables (and not the library) for Alkacon OpenCms.<br>&nbsp;<br>
Uncheck this option if already existing tables should be used.
<%= Bean.getHtmlPart("C_HELP_END") %>

<%= Bean.getHtmlPart("C_HELP_START", "6") %>
<b>AS400 configuration notes:</b><br>&nbsp;<br>
The Setup Wizard does not create the user, so be sure to create the user before continuing.<br>&nbsp;<br>
