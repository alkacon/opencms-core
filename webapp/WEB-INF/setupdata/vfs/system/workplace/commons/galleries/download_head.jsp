...
<!--
	...
	function upload() {
		...
		top.gallery_fs.gallery_list.location.href="<%=wp.getJsp().link(wp.C_PATH_DIALOGS+OpenCms.getWorkplaceManager().getExplorerTypeSetting("upload").getNewResourceUri())%>?<%=CmsNewResourceUpload.PARAM_REDIRECTURL%>=/system/workplace/commons/galleries/download_head.jsp&<%=CmsNewResourceUpload.PARAM_TARGETFRAME%>=gallery_list&<%=CmsNewResource.PARAM_CURRENTFOLDER%>="+currentfolder;
	}
//-->
</script>
...
<form name="main" action="<%= cms.link("download_fs_sub.jsp") %>" target="gallery_fs" method="post" class="nomargin">
...
<tr>
	<td colspan="2"><%= wp.key("button.downloadlist") %></td>
</tr>
<tr>
