...
%><%= wp.htmlStart(null) %>
	...
	<!--
		function validateGalleryPath() {
			...
			top.preview_fs.gallery_buttonbar.location.href="<%=wp.getJsp().link("img_buttonbar.jsp")%>?<%= wp.PARAM_FIELDID %>="+field_id+"&<%= wp.PARAM_DIALOGMODE %>="+dialogmode+"&resourcepath="+top.gallery_fs.gallery_head.previewUri;
			top.preview_fs.gallery_preview.location.href="<%=wp.getJsp().link("img_preview.jsp")%>?resourcepath="+top.gallery_fs.gallery_head.previewUri;
		}
		
		function preview(uri) {
			top.gallery_fs.gallery_head.previewUri = uri;
			top.preview_fs.gallery_buttonbar.location.href="<%=wp.getJsp().link("img_buttonbar.jsp")%>?<%= wp.PARAM_FIELDID %>=<%= wp.getParamFieldId() %>&<%= wp.PARAM_DIALOGMODE %>=<%= wp.getParamDialogMode() %>&resourcepath="+uri;
