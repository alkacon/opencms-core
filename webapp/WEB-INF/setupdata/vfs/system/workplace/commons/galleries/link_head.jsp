...
<!--
	...
	function create() {
		...
		top.gallery_fs.gallery_list.location.href="<%=wp.getJsp().link(wp.C_PATH_DIALOGS+OpenCms.getWorkplaceManager().getExplorerTypeSetting(CmsResourceTypePointer.C_RESOURCE_TYPE_NAME).getNewResourceUri())%>?action=newform&<%=CmsNewResourceUpload.PARAM_REDIRECTURL%>=<%=wp.C_PATH_DIALOGS%>galleries/link_head.jsp&<%=CmsNewResourceUpload.PARAM_TARGETFRAME%>=gallery_list&<%=CmsNewResource.PARAM_CURRENTFOLDER%>="+currentfolder;		
	}
//-->
</script>
...
<form name="main" action="<%= cms.link("link_fs_head.jsp") %>" target="gallery_fs" method="post" class="nomargin">
...
<tr>
	<td colspan="2"><%= wp.galleriesExists()==true?"":wp.key("error.reason.linknogallery") %><%= wp.buildGallerySelectBox() %></td>
</tr>
<tr>
	<td class="maxwidth">
