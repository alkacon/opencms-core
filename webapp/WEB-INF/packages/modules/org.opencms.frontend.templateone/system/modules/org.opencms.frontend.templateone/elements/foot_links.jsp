...
if (CmsTemplateBean.C_PARAM_ACCESSIBLE.equals(cms.getLayout())) {
	...
	<% if (showLogin) { %>
	| <a class="linksheadfoot" href="<%= cms.link(loginUri) %>" title="<%= loginText %>"><%= loginText %></a>
	<% } %>
	...
	<% if (showLogin) { %>
	<a href="<%= cms.link(loginUri) %>" title="<%= loginText %>"><img height="14" alt="<%= loginText %>" src="<%= cms.getResourcePath() %>login.gif" width="19" align="middle" border="0" /></a>
	<% } %>
