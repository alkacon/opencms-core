...
Adds the showElementAvailability preference
...
<xsl:param name="opencmsWorkplace" select="document(concat($configDir, '/defaults/opencms-workplace.xml'))" />
    ...
    <xsl:template match="preference-tab[@name='basic']">
        <preference-tab name="basic">
            <xsl:apply-templates />
            <xsl:if test="not(../preference-tab/preference[@name='showElementAvailability'])">
                <xsl:copy-of select="$opencmsWorkplace//preference[@name='showElementAvailability']" />
            </xsl:if>
        </preference-tab>
    </xsl:template>
</xsl:stylesheet>
