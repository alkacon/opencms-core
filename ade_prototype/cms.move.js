(function(cms) {
   ...
   var initContainerForDrag = cms.move.initContainerForDrag = function(sortable, container) {
      ...
      if (!cms.data.elements[sortable.cmsResource_id].contents[containerType]) 
         return;
      saveZIndex(container.id);
      
      if (container.id != sortable.cmsStartContainerId) {
         sortable.cmsHoverList += ', #' + container.id;
         var helperElem;
         ...
         sortable.cmsHelpers[container.id] = helperElem.css({
            'display': 'none',
            'position': 'absolute',
            'opacity': sortable.options.opacity,
            'zIndex': sortable.options.zIndex
         }).addClass('ui-sortable-helper cms-element').attr('rel', sortable.cmsItem.id).appendTo('#' + container.id);
         var $handle = $('<a class="cms-handle cms-move"></a>').appendTo(sortable.cmsHelpers[container.id]);
         if (sortable.cmsStartContainerId != cms.toolbar.currentMenuItems) {
            $handle.mousedown(movePreparation).mouseup(moveEnd);
         }
         
      } else {
         sortable.cmsHelpers[container.id] = sortable.helper;
   ...
   var startDragFromMenu = cms.move.startDragFromMenu = function(sortable) {
      ...
      var loadedItem = cms.data.elements['item_001'];
      //      if (sortable.cmsStartContainerId == 'cms-search-list') 
      //         loadDynamicElement(sortable, loadedItem);
