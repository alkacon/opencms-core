...
public abstract class TestCmsMessageBundles extends TestCase {
    ...
    private List getExludedLocalisedBundles() {
        ...
        if (m_excludedBundles == null) {
            ...
            for (int i = 0; i < getTestMessageBundles().length; i++) {
                ...
                CmsMessages messages = new CmsMessages(bundle.getBundleName(),
                        Locale.ENGLISH);
    ...
    protected CmsMessages getMessageBundle(I_CmsMessageBundle bundle, Locale locale) throws IOException {
        ...
        String fileName = CmsStringUtil.substitute(bundle.getBundleName(), ".", "/") + "_" 
    ...
    private void doPreTestBundle(I_CmsMessageBundle bundle) {
        ...
        if (!className.toLowerCase().equals(bundle.getBundleName())) {
            fail("bundle name '" + bundle.getBundleName()
                    + "' has not the form: packagename.messages");
        }
    ...
    private void doTestBundle(I_CmsMessageBundle bundle, Locale locale) throws Exception {
        ...
        System.out.println("\nValidating all keys in bundle "
                + bundle.getBundleName() + " for locale " + locale + ":");
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (field.getType().equals(String.class)) {
                ...
                if (!key.equals(value)) {
                    errorMessages.add("Key '" + key + "' has bad value '"
                            + value + "'");
                }
                ...
                if (key.length() < 7) {
                    errorMessages.add("Key '" + key
                            + "' is too short (length must be at least 7)");
                }
                ...
                if ((key.charAt(key.length() - 2) != '_')
                        || (!key.startsWith("ERR_") && !key.startsWith("LOG_")
                                ...
                                && !key.startsWith("GUI_") && !key
                                .startsWith("RPT_"))) {
                    errorMessages.add("Key '" + key
                                    + "' must have the form {ERR|LOG|INIT|GUI|RPT}_KEYNAME_{0-9}");
                }
                int argCount = Integer.valueOf(key.substring(key.length() - 1))
                        .intValue();
            ...
            while (bundleKeys.hasMoreElements()) {
                ...
                if (!bundleKey.toUpperCase().equals(bundleKey)) {
                    ...
                    for (int j = 0; j < argCount; j++) {
                        ...
                        if (pos < 0) {
                            errorMessages.add("Message '" + keyValue + "' for key '" + bundleKey
                                    + " misses argument {" + j + "}");
                        }
                    }
                    for (int j = argCount; j < 10; j++) {
        ...
        if (!errorMessages.isEmpty()) {
            String msg = "Errors for bundle name '" + bundle.getBundleName()
                    + "' and Locale '" + locale + "'";
