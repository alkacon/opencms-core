...
/**
 * This class describes the synchronize list for synchronizing the resources
...
public class CmsSynchronizeList implements I_CmsConstants, Serializable {
	...
	private void createNewLocalFile(File newFile) throws CmsException {
		...
		try {
			String pathName = newFile.getPath().substring(0, (int)newFile.getPath().lastIndexOf("/")+1);
	 ...
	 */
	public Date getSfsDate(String resourceName){
		Date lastSyncDate = null;
		Hashtable dateOfFile = (Hashtable)m_synchronizeList.get(resourceName);
		if (dateOfFile != null){
			lastSyncDate = (Date)dateOfFile.get(C_SFS_DATE);
		}	
		return lastSyncDate;
	}	
	/**
	 ...
	 * @return Date the date of the resource
	 */
	public Date getVfsDate(String resourceName){
		Date lastSyncDate = null;
		Hashtable dateOfFile = (Hashtable)m_synchronizeList.get(resourceName);
		if (dateOfFile != null){
			lastSyncDate = (Date)dateOfFile.get(C_VFS_DATE);
		}	
		return lastSyncDate;
	}
	/**
	...
	public void putDates(String resourceName, long vfsDate, long sfsDate){
		...
		dateOfFile.put(C_VFS_DATE, new Date(vfsDate));
		dateOfFile.put(C_SFS_DATE, new Date(sfsDate));	
	...
	public String toString(){
		...
		while (enu.hasMoreElements()){
			...
			date = (Date)value.get(C_VFS_DATE);
			dateString = (String)(date.getDate()+"."+(date.getMonth()+1)+"."+(date.getYear()+1900)+" "+date.getHours()+":"+date.getMinutes());
			//dateString = (String)(date.toString());
			output.append(dateString+", SFS ");
			date = (Date)value.get(C_SFS_DATE);
			dateString = (String)(date.getDate()+"."+(date.getMonth()+1)+"."+(date.getYear()+1900)+" "+date.getHours()+":"+date.getMinutes());
