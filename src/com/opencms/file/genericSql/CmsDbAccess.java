 ...
 * @author Finn Nielsen
 * @version $Revision: 1.163 $ $Date: 2000/10/26 09:58:54 $ * 
 */
public class CmsDbAccess implements I_CmsConstants, I_CmsLogChannels {
	 ...
	 * Constant to get property from configurations.
	 */
	protected static String C_CONFIGURATIONS_URL = "url";

	/**
	 * Constant to get property from configurations.
	 */
	protected static String C_CONFIGURATIONS_USER = "user";

	/**
	 * Constant to get property from configurations.
	 */
	protected static String C_CONFIGURATIONS_PASSWORD = "password";
	
	/**
	 * Constant to get property from configurations.
	 */
	protected static String C_CONFIGURATIONS_MAX_CONN = "maxConn";
	 ...
	 * Constant to get property from configurations.
	 */
	protected static String C_CONFIGURATIONS_GUARD = "guard";
...
>>>>>>> 1.161
	 ...
	 public Serializable addSystemProperty(String name, Serializable object)
		...
		PreparedStatement statement=null;
		 try	{			
			...
			// create the object
			statement=m_pool.getPreparedStatement(m_cq.C_SYSTEMPROPERTIES_WRITE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_SYSTEMPROPERTIES_WRITE_KEY, statement);
			}
		  }	
	...
	public void addUserToGroup(int userid, int groupid)
		...
		if (!userInGroup(userid,groupid)) {
			...
			try {
				// create statement
				statement = m_pool.getPreparedStatement(m_cq.C_GROUPS_ADDUSERTOGROUP_KEY);
			    ...
			    if( statement != null) {
					m_pool.putPreparedStatement(m_cq.C_GROUPS_ADDUSERTOGROUP_KEY, statement);
				}
			}             
		}        
	}
	   ...
	   if (file != null)
	     // create destination file
		   createFile(project,onlineProject,file,userId,parentId,destination, true);
		 	...
		 	throw new CmsException(this.getClass().getName()+".copyFile():" + source, CmsException.C_NOT_FOUND);
	...
	protected int countProperties(CmsPropertydefinition metadef)
		...
		try {
			// create statement
			statement = m_pool.getPreparedStatement(m_cq.C_PROPERTIES_READALL_COUNT_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROPERTIES_READALL_COUNT_KEY, statement);
			}
		  }
		return returnValue;		
	}
	  ...
	  } catch (SQLException se)
	  {
	  	throw new CmsException(CmsException.C_SQL_ERROR,se);
		   ...
		   try {   
				...
				statementResourceWrite = m_pool.getPreparedStatement(m_cq.C_RESOURCES_WRITE_KEY);
			...
			throw new CmsException("["+this.getClass().getName()+"] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
				...
				if( statementResourceWrite != null) {
					m_pool.putPreparedStatement(m_cq.C_RESOURCES_WRITE_KEY, statementResourceWrite);
				}
		 }
		 try {
...
public CmsFolder createFolder(CmsUser user, CmsProject project, int parentId, int fileId, String foldername, int flags) throws CmsException {
	...
	if ((oldFolder != null) && (oldFolder.getState() == C_STATE_DELETED)) {
...
public CmsLanguage createLanguage(String name, String shortName, int priority) throws com.opencms.core.CmsException
{
	 ...
	 * @param parentProject the type for the project (e.g. normal).
	 * 
	 * @exception CmsException Throws CmsException if something goes wrong.
	 */
	public CmsProject createProject(CmsUser owner, CmsGroup group, CmsGroup managergroup, 
									...
									int flags, int type, int parentProject) 
		throws CmsException {
		...
		try	{			
			...
			// write data to database     
			statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_CREATE_KEY);
			...
			statement.setInt(13,parentProject);
			statement.executeUpdate();
		 }
		catch (SQLException e){
			throw new CmsException("["+this.getClass().getName()+"]"+e.getMessage(),CmsException.C_SQL_ERROR, e);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_CREATE_KEY, statement);
			}
		}
		try {
		  CmsProject project = readProject(id);
		  if (project == null) 
		  	throw new CmsException(this.getClass().getName()+".createProject(): Project not found", CmsException.C_NOT_FOUND);
		  return project;
		} catch (SQLException se) {
			throw new CmsException(CmsException.C_SQL_ERROR,se);
		}
	...
	public CmsPropertydefinition createPropertydefinition(String name,
		...
		try {
			// create statement
			statement = m_pool.getPreparedStatement(m_cq.C_PROPERTYDEF_CREATE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROPERTYDEF_CREATE_KEY, statement);
			}
		  }
	...
	public void createSession(String sessionId, Hashtable data) 
		...
		try	{			
			...
			// write data to database     
			statement = m_pool.getPreparedStatement(m_cq.C_SESSION_CREATE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_SESSION_CREATE_KEY, statement);
			}
		}
	}
	/**
	...
	public CmsTask createTask(int rootId, int parentId, int tasktype, 
		...
		try {
			...
			statement = m_pool.getPreparedStatement(m_cq.C_TASK_TYPE_COPY_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASK_TYPE_COPY_KEY, statement);
			}
		}		
		return task;
	}
	...
	public void deleteAllProperties(int resourceId)
		...
		try {
			// create statement
			statement = m_pool.getPreparedStatement(m_cq.C_PROPERTIES_DELETEALL_KEY);
			statement.setInt(1, resourceId);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROPERTIES_DELETEALL_KEY, statement);
			}
		  }
	}
	/**
	 ...
	 */	
	 public void deleteFile(CmsProject project, String filename)
		 ...
		 PreparedStatement statement = null;
		 try { 
		   statement =m_pool.getPreparedStatement(m_cq.C_RESOURCES_REMOVE_KEY);  
		   ...
		   statement.executeUpdate();  
			...
			throw new CmsException("["+this.getClass().getName()+"] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
					...
					if( statement != null) {
						m_pool.putPreparedStatement(m_cq.C_RESOURCES_REMOVE_KEY, statement);
					}
		 }         
	 }
	 /**
	 ...
	 */	
	 public void deleteGroup(String delgroup)
		 ...
		 try {
			 // create statement
			 statement=m_pool.getPreparedStatement(m_cq.C_GROUPS_DELETEGROUP_KEY);
	        ...
	        if( statement != null) {
		          m_pool.putPreparedStatement(m_cq.C_GROUPS_DELETEGROUP_KEY,statement);
	        }
		 }
	 }
	 /**
	 ...
	 public void deleteProject(CmsProject project)
		 ...
		 try {			 
			 // create the statement
			 statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_DELETE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_DELETE_KEY, statement);
			}
		 }	
	 }
	/**
	...
	public void deleteProjectResources(CmsProject project)
		...
		try {
			...
			// delete all project-resources.
	      	statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_DELETE_PROJECT_KEY);
			statement.setInt(1,project.getId());       
			statement.executeQuery();           
		   ...
		   throw new CmsException("["+this.getClass().getName()+"] "+e.getMessage(),CmsException.C_SQL_ERROR, e);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_DELETE_PROJECT_KEY, statement);
			}
		  }
	}
	/**
	...
	public void deleteProperty(String meta, int resourceId, int resourceType)
		...
		if( propdef == null) {
			...
			try {
				// create statement
				statement = m_pool.getPreparedStatement(m_cq.C_PROPERTIES_DELETE_KEY);
				...
				if( statement != null) {
					m_pool.putPreparedStatement(m_cq.C_PROPERTIES_DELETE_KEY, statement);
				}
			 }
		}
	}
	...
	protected void deleteResource(int id)
		...
		try {  
			...
			statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_DELETEBYID_KEY);
			statement.setInt(1, id);
			statement.executeUpdate();
		} catch (SQLException e){
			throw new CmsException("["+this.getClass().getName()+"]"+e.getMessage(),CmsException.C_SQL_ERROR, e);			
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_DELETEBYID_KEY, statement);
			}
		}
	}
	/**
	...
	public void deleteSessions() {
		...
		try	{			
			statement = m_pool.getPreparedStatement(m_cq.C_SESSION_DELETE_KEY);
 ...
 * Creation date: (28-09-2000 11:00:11)
 * @param siteId int
 */
public void deleteSite(int siteId) throws CmsException
{
	...
	finally
	{
		if (statement != null)
		{
			m_pool.putPreparedStatement(m_cq.C_SITE_DELETESITE_KEY, statement);
		}
	}
}
	/**
 ...
 * @param siteId int
 */
public void deleteSiteReattachChildProjects(int siteId) throws CmsException
{
	PreparedStatement statement = null;
	ResultSet res = null;
	...
	{
		/*
		public Integer C_SITE_DELETESITE_REATTACH_CHILD_PROJECTS_KEY = new Integer(475);
		public String C_SITE_DELETESITE_REATTACH_CHILD_PROJECTS = "UPDATE " + C_DATABASE_PREFIX + "PROJECTS SET PARENT_ID = ? WHERE PROJECT_ID = ?";
		public Integer C_SITE_DELETESITE_FIND_ALL_CHILDREN_IDS_KEY = new Integer(476);
		public String C_SITE_DELETESITE_FIND_ALL_CHILDREN_IDS = "SELECT PROJECT_ID FROM " + C_DATABASE_PREFIX + "PROJECTS WHERE PARENT_ID = ?";
		*/
		CmsProject project = readProject(getSiteBySiteId(siteId).getOnlineProjectId());

		// Find all online projects
		Vector sites = getAllSites();
		Hashtable onlineids = new Hashtable();
		for (int i = 0; i < sites.size(); i++)
			onlineids.put(new Integer(((CmsSite) sites.elementAt(i)).getOnlineProjectId()), "");
		...
		// Update the children that are not offline projects
	...
	public void deleteSystemProperty(String name)
		...
		PreparedStatement statement = null;
		try	{
		   statement = m_pool.getPreparedStatement(m_cq.C_SYSTEMPROPERTIES_DELETE_KEY);
		   ...
		   statement.executeUpdate();   
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_SYSTEMPROPERTIES_DELETE_KEY, statement);
			}
		  }	
	}
	/**
	...
	public void deleteUser(int id) 
		...
		try	{
			statement = m_pool.getPreparedStatement(m_cq.C_USERS_DELETEBYID_KEY);
			statement.setInt(1,id);
			statement.executeUpdate();
		}
		catch (SQLException e){
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_USERS_DELETEBYID_KEY, statement);
			}
		}
	}
	/**
	...
	public void deleteUser(String name) 
		...
		try	{
			statement = m_pool.getPreparedStatement(m_cq.C_USERS_DELETE_KEY);
			statement.setString(1,name);
			statement.executeUpdate();
		}
		catch (SQLException e){
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_USERS_DELETE_KEY, statement);
			}
		}
	}
	/**
	...
	public void endTask(int taskId)
		...
		PreparedStatement statement = null;
		try{
			statement = m_pool.getPreparedStatement(m_cq.C_TASK_END_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASK_END_KEY, statement);
			}
		}
	}
/**
...
protected void fillDefaults() throws CmsException
{
	...
	protected int findAgent(int roleid)
		...
		try {
			...
			statement = m_pool.getPreparedStatement(m_cq.C_TASK_FIND_AGENT_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASK_FIND_AGENT_KEY, statement);
			}
		}
		return result;
	}
	...
	public void forwardTask(int taskId, int newRoleId, int newUserId)
		...
		PreparedStatement statement = null;
		try{
			statement = m_pool.getPreparedStatement(m_cq.C_TASK_FORWARD_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASK_FORWARD_KEY, statement);
			}
		}
	}
	/**
	 ...
	 */
	 public Vector getAllAccessibleProjectsByManagerGroup(CmsGroup group)
		 ...
		 try {			 
			 // create the statement
			 statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_READ_BYMANAGER_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_READ_BYMANAGER_KEY, statement);
			}
		 }	
		 return(projects);
	 }
 ...
 * Creation date: (22-09-2000 11:21:53)
 * @return java.util.Vector
 */
public Vector getAllCategories() throws CmsException
{
	Vector categories = new Vector();
	PreparedStatement statement = null;
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_CATEGORY_GETALLCATEGORIES_KEY);
		ResultSet res = statement.executeQuery();
		while (res.next())
			categories.addElement(new CmsCategory(res.getInt("CATEGORY_ID"), res.getString("NAME"), res.getString("DESCRIPTION"), res.getString("SHORTNAME"), res.getInt("PRIORITY")));
		res.close();
	}
	catch (SQLException e)
	{
	...
	finally
	{
		if (statement != null)
		{
			m_pool.putPreparedStatement(m_cq.C_CATEGORY_GETALLCATEGORIES_KEY, statement);
		}
	}
	return categories;
}
...
public Vector getAllLanguages() throws CmsException
{
	 ...
	 */
	 public Vector getAllProjects(int state)
		 ...
		 try {			 
			 // create the statement
			 statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_READ_BYFLAG_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_READ_BYFLAG_KEY, statement);
			}
		 }	
		 return(projects);
	 }
...
public Vector getAllSites() throws CmsException
{
	...
	try
	{
		...
		while (res.next())
		{
			sites.addElement(new CmsSite(res));
		}
		res.close();
	}
	catch (SQLException e)
 ...
 * @exception CmsException Throws CmsException if something goes wrong.
 */
public CmsCategory getCategory(int categoryId) throws com.opencms.core.CmsException 
{
	PreparedStatement statement = null;
	CmsCategory category = null;
	ResultSet res = null;
	 ...
	 public Vector getChild(String groupname) 
	  throws CmsException {
		 ...
		 try {
			...
			if (parent != null) {
				// create statement
				statement=m_pool.getPreparedStatement(m_cq.C_GROUPS_GETCHILD_KEY);
						
				statement.setInt(1,parent.getId());
				res = statement.executeQuery();
				// create new Cms group objects
		    	while ( res.next() ) {
					 group=new CmsGroup(res.getInt(m_cq.C_GROUPS_GROUP_ID),
	        ...
	        if( statement != null) {
		         m_pool.putPreparedStatement(m_cq.C_GROUPS_GETCHILD_KEY,statement);
	        }
		 }
 ...
 * Creation date: (02-10-2000 17:05:52)
 * @return com.opencms.file.CmsCountry
 * @param countryId int
 * @exception com.opencms.core.CmsException The exception description.
 */
public CmsCountry getCountry(int countryId) throws com.opencms.core.CmsException
{
	...
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_COUNTRY_SELECTCOUNTRY_KEY);
		statement.setInt(1, countryId);
		res = statement.executeQuery();
		if (res.next())
			country = new CmsCountry(countryId, res.getString("NAME"), res.getString("SHORTNAME"), res.getInt("PRIORITY"));
		else
			throw new CmsException("[" + this.getClass().getName() + "] " + countryId, CmsException.C_NOT_FOUND);
	}
	catch (SQLException e)
	{
		throw new CmsException("[" + this.getClass().getName() + "]" + e.getMessage(), CmsException.C_SQL_ERROR, e);
	...
	finally
	{
		...
		if (statement != null)
		{
			m_pool.putPreparedStatement(m_cq.C_COUNTRY_SELECTCOUNTRY_KEY, statement);
		}
	}
	return country;
}
	 ...
	 */
	 public Vector getFilesInFolder(CmsFolder parentFolder)
		 ...
		 PreparedStatement statement  = null;
			...
			statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_GET_FILESINFOLDER_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_GET_FILESINFOLDER_KEY, statement);
			}
		 }
		return files;
	 }
...
public Vector getFilesWithProperty(int projectId, String propertyDefinition, String propertyValue) throws CmsException {
	...
	PreparedStatement statement = null;
	try {
		statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_GET_FILES_WITH_PROPERTY_KEY);
		...
		if (statement != null) {
			m_pool.putPreparedStatement(m_cq.C_RESOURCES_GET_FILES_WITH_PROPERTY_KEY, statement);
		}
	}
	return names;
}
	 ...
	 public Vector getGroups() 
	  throws CmsException {
		 ...
		 try { 
	        ...
	        if( statement != null) {
		         m_pool.putPreparedStatement(m_cq.C_GROUPS_GETGROUPS_KEY,statement);
	        }
		 }
	  return groups;
	 }
	...
	/*public CmsGroup getParent(String groupname)
		...
		try{
	        ...
	        if( statement != null) {
		         m_pool.putPreparedStatement(m_cq.C_GROUPS_GETPARENT_KEY,statement);
	        }
		 }
		return parent;
	}*/
	...
	public Vector getGroupsOfUser(String name)
		...
		try {
		  //  get all all groups of the user
			statement = m_pool.getPreparedStatement(m_cq.C_GROUPS_GETGROUPSOFUSER_KEY);
			statement.setString(1,name);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_GROUPS_GETGROUPSOFUSER_KEY, statement);
			}
		}             
		return groups;
	}
 ...
 * Creation date: (02-10-2000 17:00:50)
 * @return com.opencms.file.CmsLanguage
...
public CmsLanguage getLanguage(int languageId) throws com.opencms.core.CmsException
{
	...
	try
	{
		...
		res = statement.executeQuery();
		if (res.next())
			language = new CmsLanguage(languageId, res.getString("NAME"), res.getString("SHORTNAME"), res.getInt("PRIORITY"));
		else
			throw new CmsException("[" + this.getClass().getName() + "] " + languageId, CmsException.C_NOT_FOUND);
	}
	catch (SQLException e)
	{
		throw new CmsException("[" + this.getClass().getName() + "]" + e.getMessage(), CmsException.C_SQL_ERROR, e);
 ...
 * @exception CmsException Throws CmsException if the resource is not found, or the database communication went wrong.
 */
public CmsProject getOnlineProject(int projectId) throws SQLException
{
	CmsProject res = null;
		res = getProjectIfOnline(projectId);
		if (res == null)
	  	res = getParentProject(projectId);
	  if (res == null)
	  	res = readProject(projectId);
	  return res;
}
/**
 * Read the project from the database, if the project is an online project.
 * This method is used by getOnlineProject.
 * Creation date: (10/02/00)
 * @return com.opencms.file.CmsProject the project found, or null if the given projectid was not an online project.
 * @param projectId int the project to lookup.
 * @exception java.sql.SQLException Thrown if there was any problems communicating with the database.
 */
public CmsProject getParentProject(int projectId) throws java.sql.SQLException {
	PreparedStatement statement = null;
	CmsProject project = null;
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_PARENT_PROJECT_KEY);
		statement.setInt(1, projectId);
		ResultSet res = statement.executeQuery();
		if (res.next())
	...
	{
		//always put the prepared statement back into the pool.
		if (statement != null)
			m_pool.putPreparedStatement(m_cq.C_PROJECTS_PARENT_PROJECT_KEY, statement);
	}
	//note that the project can be null.
	return project;
}
/**
 * Read the project from the database, if the project is an online project.
 ...
 * @exception java.sql.SQLException Thrown if there was any problems communicating with the database.
 */
public CmsProject getProjectIfOnline(int projectId) throws java.sql.SQLException {
	PreparedStatement statement = null;
	CmsProject project = null;
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_ONLINE_PROJECT_KEY);
		statement.setInt(1, projectId);
		ResultSet res = statement.executeQuery();
		if (res.next())
			//SELECT SITE_ID, NAME, DESCRIPTION,CATEGORY_ID,LANGUAGE_ID, COUNTRY_ID FROM " + C_DATABASE_PREFIX + "SITES, " + C_DATABASE_PREFIX + "SITE_PROJECTS WHERE " + C_DATABASE_PREFIX + "SITE_PROJECTS.BASEPROJECT_ID = ? AND " + C_DATABASE_PREFIX + "SITE_PROJECTS.SITE_ID=" + C_DATABASE_PREFIX + "SITES.SITE_ID";
	...
	{
		if (statement != null)
			m_pool.putPreparedStatement(m_cq.C_PROJECTS_ONLINE_PROJECT_KEY, statement);
	}
	//note that the project can be null.
	return project;
}
...
public CmsSite getSiteBySiteId(int siteId) throws CmsException
{
  ...
  ResultSet res = null;
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_SITES_GETSITEFROMSITEID_KEY);
	...
	finally
	{
		...
		if (statement != null)
		{
			m_pool.putPreparedStatement(m_cq.C_SITES_GETSITEFROMSITEID_KEY, statement);
		}
	}
	return site;
}
...
/**
 * Returns all sites in system with the needed info to create a sitematrix
 * Creation date: (07-09-2000 13:45:00)
 * @author Finn Nielsen
 * @return Vector
 */
public Vector getSiteMatrixInfo() throws CmsException
{
	...
	try
	{
		...
		//"SELECT SITE_ID, " + 
		//C_DATABASE_PREFIX + "SITES.CATEGORY_ID, " + 
		//C_DATABASE_PREFIX + "SITES.LANGUAGE_ID, " + 
		//C_DATABASE_PREFIX + "SITES.COUNTRY_ID, " + 
		//C_DATABASE_PREFIX + "LANGUAGE.SHORTNAME AS LANG_NAME, " + 
		//C_DATABASE_PREFIX + "COUNTRY.SHORTNAME AS COUNTRY_NAME 
		//FROM " + C_DATABASE_PREFIX + "SITES," + C_DATABASE_PREFIX + "COUNTRY," + C_DATABASE_PREFIX + "LANGUAGE WHERE " + C_DATABASE_PREFIX + "SITES.LANGUAGE_ID=" + C_DATABASE_PREFIX + "LANGUAGE.LANGUAGE_ID AND " + C_DATABASE_PREFIX + "SITES.COUNTRY_ID=" + C_DATABASE_PREFIX + "COUNTRY.COUNTRY_ID";
		ResultSet res = statement.executeQuery();
		while (res.next())
		{
...
public Vector getSiteUrls(int siteId) throws com.opencms.core.CmsException
{
	 ...
	 */
	 public Vector getSubFolders(CmsFolder parentFolder)
		 ...
		 PreparedStatement statement = null;
		   ...
		   try {
			 //  get all subfolders
			 statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_GET_SUBFOLDER_KEY);
			 statement.setInt(1,parentFolder.getResourceId()); 
			...
			if( statement != null) {
				...
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_GET_SUBFOLDER_KEY, statement);
			}
		 }
		return folders; 
	 }
	...
	public String getTaskPar(int taskId, String parname)
		...
		try {
			statement = m_pool.getPreparedStatement(m_cq.C_TASKPAR_GET_KEY);
			statement.setInt(1, taskId);
			statement.setString(2, parname);
			res = statement.executeQuery();
			if(res.next()) {
				result = res.getString(m_cq.C_PAR_VALUE);
			}
			res.close();			
		} catch( SQLException exc ) {
			throw new CmsException(exc.getMessage(), CmsException.C_SQL_ERROR, exc);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASKPAR_GET_KEY, statement);
			}
		}
		return result;
	}
	...
	public int getTaskType(String taskName)
		...
		{
		case C_TASKS_ALL: {
				result = result + m_cq.C_TASK_ROOT + "<>0";			
				break;				
			}
		case C_TASKS_OPEN: {
				result = result + m_cq.C_TASK_STATE + "=" + C_TASK_STATE_STARTED;
				break;
			}	
		case C_TASKS_ACTIVE: {
				result = result + m_cq.C_TASK_STATE + "=" + C_TASK_STATE_STARTED;
		...
		case C_TASKS_DONE: {
				result = result + m_cq.C_TASK_STATE + "=" + C_TASK_STATE_ENDED;
				break;					
			}
		case C_TASKS_NEW: {
	...
	public Vector getUsersOfGroup(String name, int type)
		...
		try {
			statement = m_pool.getPreparedStatement(m_cq.C_GROUPS_GETUSERSOFGROUP_KEY);
			statement.setString(1,name);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_GROUPS_GETUSERSOFGROUP_KEY, statement);
			}
		}             
		return users;
	}
	...
	protected void initId() 
		...
		try {
			statement = m_pool.getPreparedStatement(m_cq.C_SYSTEMID_INIT_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_SYSTEMID_INIT_KEY, statement);
			}
		}
	}
/**
...
protected void initIdStatements() throws com.opencms.core.CmsException {
	...
	protected void initMaxIdValues() 
		...
		m_maxIds[C_TABLE_GROUPS] = initMaxId(m_cq.C_GROUPS_MAXID_KEY);
		m_maxIds[C_TABLE_PROJECTS] = initMaxId(m_cq.C_PROJECTS_MAXID_KEY);
		m_maxIds[C_TABLE_USERS] = initMaxId(m_cq.C_USERS_MAXID_KEY);
		m_maxIds[C_TABLE_SYSTEMPROPERTIES] = initMaxId(m_cq.C_SYSTEMPROPERTIES_MAXID_KEY);
		m_maxIds[C_TABLE_PROPERTYDEF] = initMaxId(m_cq.C_PROPERTYDEF_MAXID_KEY);
		m_maxIds[C_TABLE_PROPERTIES] = initMaxId(m_cq.C_PROPERTIES_MAXID_KEY);
		m_maxIds[C_TABLE_RESOURCES] = initMaxId(m_cq.C_RESOURCES_MAXID_KEY);		
		m_maxIds[C_TABLE_FILES] = initMaxId(m_cq.C_FILES_MAXID_KEY);		
	...
	protected void initStatements() 
		...
		m_pool.initPreparedStatement(m_cq.C_SITES_GETSITEFROMSITEID_KEY, m_cq.C_SITES_GETSITEFROMSITEID);
		...
		m_pool.initPreparedStatement(m_cq.C_PROJECTS_ONLINE_PROJECT_KEY, m_cq.C_PROJECTS_ONLINE_PROJECT);
		...
		m_pool.initPreparedStatement(m_cq.C_SITE_DELETESITE_FIND_ALL_CHILDREN_IDS_KEY, m_cq.C_SITE_DELETESITE_FIND_ALL_CHILDREN_IDS);
		m_pool.initPreparedStatement(m_cq.C_SITE_DELETESITE_REATTACH_CHILD_PROJECTS_KEY, m_cq.C_SITE_DELETESITE_REATTACH_CHILD_PROJECTS);
		...
		m_pool.initPreparedStatement(m_cq.C_LANGUAGE_SELECTLANGUAGE_KEY, m_cq.C_LANGUAGE_SELECTLANGUAGE);
		m_pool.initPreparedStatement(m_cq.C_COUNTRY_SELECTCOUNTRY_KEY, m_cq.C_COUNTRY_SELECTCOUNTRY);
		m_pool.initPreparedStatement(m_cq.C_SITES_CHECKSITE_KEY, m_cq.C_SITES_CHECKSITE);
		m_pool.initPreparedStatement(m_cq.C_SITESSITEURLS_CHECKSITE_KEY, m_cq.C_SITESSITEURLS_CHECKSITE);
		...
		// - to be able to override method in other packages
		initIdStatements();
	...
	protected int insertTaskPar(int taskId, String parname, String parvalue) 
		...
		try {
			...
			statement = m_pool.getPreparedStatement(m_cq.C_TASKPAR_INSERT_KEY);
			statement.setInt(1, newId);
 ...
 * Creation date: (04-10-2000 11:29:52)
 * @return boolean
 * @param siteId int
 * @param name java.lang.String
 * @param url java.lang.String
 ...
 * @param countryId int
 */
public boolean isSiteLegal(int siteId, String name, String url, int categoryId, int languageId, int countryId) throws CmsException
{

	//public String C_SITES_CHECKSITE = "SELECT NAME FROM " + C_DATABASE_PREFIX + "SITES WHERE NAME <> ? AND CATEGORY_ID = ? LANGUAGE_ID = ? AND COUNTRY_ID = ?";
	//public String C_SITESSITEURLS_CHECKSITE = "SELECT NAME FROM " + C_DATABASE_PREFIX + "SITE_URLS, " + C_DATABASE_PREFIX + "SITES WHERE NAME <> ? AND (LOWER(NAME) = ? OR LOWER(URL) = ?)";
	...
	{
		if (statement != null)
			m_pool.putPreparedStatement(m_cq.C_SITES_CHECKSITE_KEY, statement);
		if (statement1 != null)
			m_pool.putPreparedStatement(m_cq.C_SITESSITEURLS_CHECKSITE_KEY, statement1);
	}
	return isLegal;
}
/**
 ...
 */
public void newSiteProjectsRecord(int siteID, int projectId) throws CmsException
{
	PreparedStatement statement = null;
	try
	{
...
{
	try //catch SQLException form readFolder, etc.
		...
		for (int i = 0; i < offlineFolders.size(); i++)
		{
			...
			if (currentFolder.getState() == C_STATE_DELETED)
			{
				...
				if (currentFolder.getState() == C_STATE_NEW)
				{
					...
					if (parentId == null)
					{
						CmsFolder currentOnlineParent = null;
						currentOnlineParent = readFolder(onlineProject.getId(), currentFolder.getParent());
						if (currentOnlineParent == null)
							throw new CmsException(this.getClass().getName() + ".publishProject: + Folder not found: " + currentFolder.getParent());
		...
		for (int i = 0; i < offlineFiles.size(); i++)
		{
			...
			if (currentFile.getName().startsWith(C_TEMP_PREFIX))
			{
				...
				if (currentFile.getState() == C_STATE_DELETED)
				{
					...
					CmsFile currentOnlineFile = readFile(onlineProject.getId(), currentFile.getAbsolutePath());
					try
					{
	 ...
	 */
	 public Vector readAllFileHeaders(String filename)
		 ...
		 PreparedStatement statement = null;
		 try {  
			   statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_READ_ALL_KEY);
	...
	public Hashtable readAllProperties(int resourceId, int resourceType)
		...
		try {
			// create project
			statement = m_pool.getPreparedStatement(m_cq.C_PROPERTIES_READALL_KEY);
			statement.setInt(1, resourceId);
	...
	public Vector readAllPropertydefinitions(int resourcetype, int type)
		throws CmsException {
		 ...
		 try {
			 // create statement
			 statement = m_pool.getPreparedStatement(m_cq.C_PROPERTYDEF_READALL_B_KEY);			 
			 statement.setInt(1,resourcetype);
			 statement.setInt(2,type);
			 result = statement.executeQuery();
			 while(result.next()) {
 ...
 * @param filename The complete name of the new file (including pathinformation).
 * 
 * @return file The read file, or null if the resource could not be found.
 * Note that if a resource has been deleted, the file is still returned.
 * 
 * @exception CmsException Throws CmsException if operation was not succesful
 */
public CmsFile readFile(int projectId, String filename) throws SQLException, CmsException
{
	CmsFile file = null;
	PreparedStatement statement = null;
	ResultSet res = null;
	try
	{
		...
		//if the file was not found, or had been deleted - just return it.
		...
		if (res.next())
			...
			//if the resource was not deleted - we should be able to read its content. If not this is a serious error.
	...
	protected byte[] readFileContent(int fileId)
		...
		try {  
			...
			res = statement.executeQuery();
			if (res.next()) {
				  returnValue = res.getBytes(m_cq.C_FILE_CONTENT);
			} else {
	 ...
	 */
	 public CmsFile readFileHeader(int resourceId)
		 ...
		 PreparedStatement statement = null;  
		 try {
			   statement=m_pool.getPreparedStatement(m_cq.C_RESOURCES_READBYID_KEY);
 ...
 * @param filename The complete name of the new file (including pathinformation).
 * 
 * @return file The read file, returns null if the resource could not be found.
 * NOTE: If the reosurce was deleted, it is still returned.
 * 
 * @exception CmsException Throws CmsException if operation was not succesful
 */
public CmsFile readFileHeader(int projectId, String filename) throws SQLException
{
	CmsFile file = null;
	ResultSet res = null;
	PreparedStatement statement = null;
	try
	{
		statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_READ_KEY);
	 ...
	 */
	 public Vector readFiles(int projectId)
		 ...
		 try {  
			   // read file data from database
			   statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_READFILESBYPROJECT_KEY);
			   statement.setInt(1,projectId);
			   res = statement.executeQuery();
	 ...
	 * 
	 * @return The read folder, null if not found.
	 * 
	 * @exception CmsException Throws CmsException if operation was not succesful.
	 */
	 ...
	 */
	 public Vector readFolders(int projectId)
		 ...
		 try {  
			   ...
			   statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_READFOLDERSBYPROJECT_KEY);
			   statement.setInt(1,projectId);
			   res = statement.executeQuery();
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_READFOLDERSBYPROJECT_KEY, statement);
			}
		}
		return folders;
	 }
	 ...
	 */
	 public CmsGroup readGroup(int id)
		 ...
		 try{ 
			 ...
			 statement=m_pool.getPreparedStatement(m_cq.C_GROUPS_READGROUP2_KEY);
			 statement.setInt(1,id);
			 res = statement.executeQuery();           
			 // create new Cms group object
			 if(res.next()) {     
		 ...
		 throw new CmsException("[" + this.getClass().getName() + "] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
	        ...
	        if( statement != null) {
		         m_pool.putPreparedStatement(m_cq.C_GROUPS_READGROUP2_KEY,statement);
	        }
		 }
		 return group;
	 }
	 ...
	 */
	 public CmsGroup readGroup(String groupname)
		 ...
		 try{ 
			 // read the group from the database
			 statement=m_pool.getPreparedStatement(m_cq.C_GROUPS_READGROUP_KEY);
	 ...
	 * @exception CmsException Throws CmsException if something goes wrong.
	 */
	public CmsProject readProject(int id) throws SQLException {
		...
		try	{			
			statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_READ_KEY);
			statement.setInt(1,id);
			res = statement.executeQuery();
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_READ_KEY, statement);
			}
		}
		return project;
	}
	...
	public CmsProject readProject(CmsTask task) 
		...
		try	{			
			...
			if( statement != null) 
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_READ_BYTASK_KEY, statement);
		}
		return project;
	}
	/**
	...
	public Vector readProjectLogs(int projectid)
		...
		try {
			statement = m_pool.getPreparedStatement(m_cq.C_TASKLOG_READ_PPROJECTLOGS_KEY);
			statement.setInt(1, projectid);
			res = statement.executeQuery();
			while(res.next()) {				 
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASKLOG_READ_PPROJECTLOGS_KEY, statement);
			}
		}
		return logs;
	}
	...
	public CmsPropertydefinition readPropertydefinition(String name, int type)
		throws CmsException {
		 ...
		 try {
			...
			// if resultset exists - return it
			if(res.next()) {
			    propDef = new CmsPropertydefinition( res.getInt(m_cq.C_PROPERTYDEF_ID),
			   								res.getString(m_cq.C_PROPERTYDEF_NAME),
			   								res.getInt(m_cq.C_PROPERTYDEF_RESOURCE_TYPE),
			   								res.getInt(m_cq.C_PROPERTYDEF_TYPE) );
			    res.close();
			} else {
				res.close();
			    // not found!
	 ...
	 */
	 public Vector readResources(CmsProject project)
		 ...
		 try {  
			   // read resource data from database
			   statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_READBYPROJECT_KEY);
			   statement.setInt(1,project.getId());
			   res = statement.executeQuery();
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_READBYPROJECT_KEY, statement);
			}
		}
		return resources;
	 }
	...
	public Serializable readSystemProperty(String name)
		...
		try {
		  ...
		  res = statement.executeQuery();
		  if(res.next()) {
				value = res.getBytes(m_cq.C_SYSTEMPROPERTY_VALUE);
				// now deserialize the object
				ByteArrayInputStream bin= new ByteArrayInputStream(value);
				ObjectInputStream oin = new ObjectInputStream(bin);
...
public CmsTask readTask(int id) throws CmsException {
	...
	try {
		...
		res = statement.executeQuery(m_cq.C_TASK_READ_STATEMENT+id);  
		if (res.next()) { 
	...
	public CmsTaskLog readTaskLog(int id)
		...
		try {
			statement = m_pool.getPreparedStatement(m_cq.C_TASKLOG_READ_KEY);
			statement.setInt(1, id);
			res = statement.executeQuery();
			if(res.next()) {				 
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASKLOG_READ_KEY, statement);
			}
		}
	...
	public void recoverPassword(String user, String recoveryPassword, String password ) 
		...
		try	{			
			statement = m_pool.getPreparedStatement(m_cq.C_USERS_RECOVERPW_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_USERS_RECOVERPW_KEY, statement);
			}
		}
		if(result != 1) {
		...
		throws CmsException{
		 ...
		 if (files.size()==0) {
		     ...
		     PreparedStatement statement = null;
				 try {          
					// delete the folder
		            statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_ID_DELETE_KEY);
					...
					if( statement != null) {
						m_pool.putPreparedStatement(m_cq.C_RESOURCES_ID_DELETE_KEY, statement);
					}
				  } 
			 } else {
	 ...
	 public void removeUserFromGroup(int userid, int groupid)
		 ...
		 try {
			...
			throw new CmsException("[" + this.getClass().getName() + "] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
			 ...
			 if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_GROUPS_REMOVEUSERFROMGROUP_KEY, statement);
			}            
		}
	 }
	 /**
	 ...
	 */		
	 public void renameFile(CmsProject project,
		 ...
		 PreparedStatement statement = null;
		 try{
		    statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_RENAMERESOURCE_KEY);
			...
			statement.executeUpdate();
			  ...
			  throw new CmsException("["+this.getClass().getName()+"] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
				...
				if( statement != null) {
					m_pool.putPreparedStatement(m_cq.C_RESOURCES_RENAMERESOURCE_KEY, statement);
				}
			}                
	 }
	/**
	...
	public void setPassword(String user, String password) 
		...
		try	{			
			statement = m_pool.getPreparedStatement(m_cq.C_USERS_SETPW_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_USERS_SETPW_KEY, statement);
			}
		}
	}
	/**
	...
	public void setRecoveryPassword(String user, String password) 
		...
		try	{			
			statement = m_pool.getPreparedStatement(m_cq.C_USERS_SETRECPW_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_USERS_SETRECPW_KEY, statement);
			}
		}
		if(result != 1) {
			// the update wasn't succesfull -> throw exception
...
protected Vector SortEntrys(Vector list)
{
	...
	public void unlockProject(CmsProject project)
		throws CmsException {
		 ...
		 try {			 
			 // create the statement
			 statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_UNLOCK_KEY);
			 statement.setInt(1,project.getId());
			 statement.executeUpdate();
		 } catch( Exception exc ) {
			 throw new CmsException("[" + this.getClass().getName() + "] " + exc.getMessage(), 
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_UNLOCK_KEY, statement);
			}
		 }	
	}
/**
...
public void updateLockstate(CmsResource res) throws CmsException {
		...
		try {
			...
			statement = m_pool.getPreparedStatement(m_cq.C_RESOURCES_UPDATE_LOCK_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_RESOURCES_UPDATE_LOCK_KEY, statement);
			}
		}
...
public void updateSite(int siteId, String name, String description, int categoryId, int languageId, int countryId, String url) throws com.opencms.core.CmsException
{
	...
	protected void updateTaskPar(int parid, String parname, String parvalue) 
		...
		try {
			...
			statement = m_pool.getPreparedStatement(m_cq.C_TASKPAR_UPDATE_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASKPAR_UPDATE_KEY, statement);
			}
		}
	}
	/**
	 ...
	 */
	 public boolean userInGroup(int userid, int groupid)
		...
		try {
			// create statement
			statement = m_pool.getPreparedStatement(m_cq.C_GROUPS_USERINGROUP_KEY);
			...
			throw new CmsException("[" + this.getClass().getName() + "] "+e.getMessage(),CmsException.C_SQL_ERROR, e);			
			 ...
			 if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_GROUPS_USERINGROUP_KEY, statement);
			}            
		}
		 return userInGroup;
	 }
	 ...
	 */	
	 public void writeGroup(CmsGroup group)
		 ...
		 try {
	        ...
	        if( statement != null) {
		         m_pool.putPreparedStatement(m_cq.C_GROUPS_WRITEGROUP_KEY,statement);
	        }
		 }
	 }
	 /**
	 ...
	 public void writeProject(CmsProject project)
		 ...
		 try {			 
			 // create the statement
			 statement = m_pool.getPreparedStatement(m_cq.C_PROJECTS_WRITE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_PROJECTS_WRITE_KEY, statement);
			}
		 }	
	 }
	/**
	...
	public Serializable writeSystemProperty(String name, Serializable object)
		...
		try	{			
			...
			statement=m_pool.getPreparedStatement(m_cq.C_SYSTEMPROPERTIES_UPDATE_KEY);
			...
			if( statement != null) {
				m_pool.putPreparedStatement(m_cq.C_SYSTEMPROPERTIES_UPDATE_KEY, statement);
			}
		  }
	...
	public CmsTask writeTask(CmsTask task)
		...
		try {    
			statement = m_pool.getPreparedStatement(m_cq.C_TASK_UPDATE_KEY);
			...
			if(statement != null) {
				m_pool.putPreparedStatement(m_cq.C_TASK_UPDATE_KEY, statement);
			}
		}
