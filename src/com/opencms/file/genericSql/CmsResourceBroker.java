 ...
 * @version $Revision: 1.163 $ $Date: 2000/10/11 12:43:30 $
 * 
 */
...
public boolean accessRead(CmsUser currentUser, CmsProject currentProject, CmsResource resource) throws CmsException
{
	...
	// check the access to the project
	if (!accessProject(currentUser, currentProject, project))		
		// no access to the project!
		return false;
	...
	CmsResource res = resource; // save the original resource name to be used is an error occurs.
	while (res.getParent() != null)
	{
		if (!accessOther(currentUser, currentProject, resource, C_ACCESS_PUBLIC_READ) && !accessOwner(currentUser, currentProject, resource, C_ACCESS_OWNER_READ) && !accessGroup(currentUser, currentProject, resource, C_ACCESS_GROUP_READ))
			return false;
		System.out.println("Parent to: " + res.getName() + " is: " + res.getParent());
		if (res.getParent() != null)
		{
			//read the folder. We dont want to test for accessRead, so do it directly ourself.
			CmsResource _res = (CmsFolder) m_resourceCache.get(C_FOLDER + res.getProjectId() + res.getParent());
			if (_res == null)
			{
				try
				{
					_res = m_dbAccess.readFolder(res.getProjectId(), res.getParent());
				}
				...
				if (_res != null)
					m_resourceCache.put(C_FOLDER + res.getProjectId() + res.getAbsolutePath(), _res);
			}
			res = _res;
