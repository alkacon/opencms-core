 ...
 * @version $Revision: 1.1 $ $Date: 2001/01/30 14:00:51 $
 */
public class CmsSynchronize implements I_CmsConstants{
	...
	public CmsSynchronize(CmsObject cms, String resourceName) 
		...
		if ((resourceName != null) && (m_synchronizePath != null)){
			...
			File syncpath = new File(m_synchronizePath);
			if (!syncpath.exists()){
				createDirectories(syncpath);
			}
	...
	private void synchronizeServer(String resourceName)
		...
		if (startResource.getAbsolutePath().endsWith("/")){
			...
			File startFolder = new File(m_synchronizePath+startResource.getAbsolutePath());
			createDirectories(startFolder);
			...
			for (int i = 0; i < filesInFolder.size(); i++){
				...
				if (syncFile.getProjectId() != onlineProject){
					if (syncFile.getState() == C_STATE_DELETED) {
						...
						File sfsFile = new File(m_synchronizePath+syncFile.getAbsolutePath());
						if (sfsFile.exists()){
							sfsFile.delete();
						}
			...
			for (int i = 0; i < foldersInFolder.size(); i++){
				...
				if (syncFolder.getProjectId() != onlineProject){
					if (syncFolder.getState() == C_STATE_DELETED) {
						...
						File sfsFile = new File(m_synchronizePath+syncFolder.getAbsolutePath());
						if (sfsFile.exists()){
			...
			if (startResource.getProjectId() != onlineProject){
				if (startResource.getState() == C_STATE_DELETED) {
					// the file has to be deleted from the server filesystem
					File sfsFile = new File(m_synchronizePath+startResource.getAbsolutePath());
					if (sfsFile.exists()){
						sfsFile.delete();
					}	
	...
	private int compareDate(String resourceName, long vfsDate, long sfsDate) 
		throws CmsException{
		Date syncVfsDate = m_synchronizeList.getVfsDate(resourceName);
		Date syncSfsDate = m_synchronizeList.getSfsDate(resourceName);
		Date fileVfsDate = new Date(vfsDate);
		Date fileSfsDate = new Date(sfsDate);
		if (syncSfsDate == null){
			return C_SYNC_SFS;
		}	
		if (fileVfsDate.after(syncVfsDate)){
			if (fileSfsDate.after(syncSfsDate)){
	...
	/**
	 * Create all directories for the given file
	 * 
	 * @param newFile the file for which the directories have to be created.
	 * @return boolean is true if the directories could be created
	 */
	private boolean createDirectories(File newFile){
		// first read the last folder of the path and put it on the stack
		File filePath = new File(newFile.getPath().substring(0, newFile.getPath().lastIndexOf("/")+1));
		Stack directories = new Stack();
		directories.push(filePath);
		// now read the parent folder
		String pathName = filePath.getPath().substring(0, filePath.getPath().lastIndexOf("/"));
		pathName = pathName.substring(0, pathName.lastIndexOf("/")+1);	
		File nextPath = new File(pathName);
		// put the parent folder on the stack and read the next parent folder
		while (filePath.getPath().indexOf("/") != nextPath.getPath().lastIndexOf("/")){
			directories.push(nextPath);
			pathName = pathName.substring(0, pathName.lastIndexOf("/"));
			pathName = pathName.substring(0, pathName.lastIndexOf("/")+1);				
			nextPath = new File(pathName);
		}
		// create all the folders in the stack beginning with the root
		while (directories.size() > 0){
			filePath = (File)directories.pop();
			filePath.mkdir();	
		}
		// if all directories could be created and the last folder now exists, return true
		filePath = new File(newFile.getPath().substring(0, newFile.getPath().lastIndexOf("/")+1));
	...
	private boolean deleteDirectory (File delFolder){
		...
		for (int i = 0; i < diskFiles.length; i++){
			...
			if (currentFile.isDirectory()){
				...
				resourceName = currentFile.getParent().substring(m_synchronizePath.length())+currentFile.getName()+"/";
			} else {
				if (!currentFile.delete()){
					return false;
				}
				resourceName = currentFile.getParent().substring(m_synchronizePath.length(), currentFile.getParent().lastIndexOf("/")+1)+currentFile.getName();
