 ...
 * @version $Revision: 1.4 $ $Date: 2001/01/24 09:42:14 $
 */
public class CmsClock extends CmsXmlTemplate {
     ...
     */
    public Object get(CmsObject cms, String tagcontent, A_CmsXmlContent doc, Object userObject) throws CmsException {
        ...
        if("year".equals(tagcontent.toLowerCase())) {
            ...
            if("month".equals(tagcontent.toLowerCase())) {
                
                // return the month
                return twoDigit(now.get(Calendar.MONTH) + 1);
            }
            else {
                if("day_of_month".equals(tagcontent.toLowerCase())) {
                    
                    // return the day of month
                    return twoDigit(now.get(Calendar.DAY_OF_MONTH));
                }
                else {
                    if("hour".equals(tagcontent.toLowerCase())) {
                        
                        // return the hour
                        return twoDigit(now.get(Calendar.HOUR_OF_DAY));
                    }
                    else {
                        if("minute".equals(tagcontent.toLowerCase())) {
                            
                            // return the minute
                            return twoDigit(now.get(Calendar.MINUTE));
                        }
                        else {
                            if("second".equals(tagcontent.toLowerCase())) {
