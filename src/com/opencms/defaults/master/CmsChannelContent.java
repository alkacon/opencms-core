...
public class CmsChannelContent extends A_CmsContentDefinition implements I_CmsExtendedContentDefinition{
    ...
    public CmsChannelContent(CmsObject cms, CmsUUID channelId) throws CmsException {
        ...
        try {
            ...
            if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO)) {
                OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Could not get channel " + channelId);
            }
        } finally {
    ...
    public void undelete(CmsObject cms) throws Exception {
        ...
        try{
            ...
            if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
                OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Could not undelete channel "+cms.readAbsolutePath(m_channel));
            }
        } finally {
            cms.getRequestContext().restoreSiteRoot();
        }
    ...
    public void publishResource(CmsObject cms) {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Channels can't be published directly!");
        }
    }
    ...
    public void restore(CmsObject cms, int versionId) {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Channels can't be restored from history!");
        }
    }
    ...
    public void chown(CmsObject cms, CmsUUID owner) {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Permissions of Channels can be changed only in EditBackoffice!");
        }
    }
    ...
    public CmsUUID copy(CmsObject cms) {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Channels can be copied!");
        }
...
//            }
    ...
    public Vector getHistory(CmsObject cms) {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO) ) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsChannelContent] Channels have no history!");
        }
        return null;
    }
