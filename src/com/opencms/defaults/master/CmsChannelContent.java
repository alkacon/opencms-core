...
public class CmsChannelContent extends A_CmsContentDefinition implements I_CmsExtendedContentDefinition{
    ...
    public CmsChannelContent(CmsObject cms, CmsUUID channelId) throws CmsException {
        ...
        try {
            ...
            if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled()) {
                OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Could not get channel " + channelId);
            }
        } finally {
    ...
    public void undelete(CmsObject cms) throws Exception {
        ...
        try{
            ...
            if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
                OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Could not undelete channel "+cms.readAbsolutePath(m_channel));
            }
        } finally {
            cms.getRequestContext().restoreSiteRoot();
        }
    ...
    public void publishResource(CmsObject cms) {
        if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
            OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Channels can't be published directly!");
        }
    }
    ...
    public void restore(CmsObject cms, int versionId) {
        if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
            OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Channels can't be restored from history!");
        }
    }
    ...
    public void chown(CmsObject cms, CmsUUID owner) {
        if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
            OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Permissions of Channels can be changed only in EditBackoffice!");
        }
    }
    ...
    public CmsUUID copy(CmsObject cms) {
        if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
            OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Channels can be copied!");
        }
...
//            }
    ...
    public Vector getHistory(CmsObject cms) {
        if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isInfoEnabled() ) {
            OpenCms.getLog(CmsLog.CHANNEL_MAIN).info("[CmsChannelContent] Channels have no history!");
        }
        return null;
    }
    ...
    public static Vector getChannelList(CmsObject cms) throws CmsException {
        ...
        try {
            ...
            if (OpenCms.getLog(CmsLog.CHANNEL_MODULE).isWarnEnabled()) {
                OpenCms.getLog(CmsLog.CHANNEL_MODULE).warn("[CmsChannelContent]: error when reading subfolders of cos root: "+e.getMessage());
            }
        } finally {
            cms.getRequestContext().restoreSiteRoot();
        }
