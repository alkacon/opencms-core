...
public class CmsDbAccess {
        ...
        try {
            ...
            for (int i = 0; i < channelToAdd.size(); i++) {
                try {
                    ...
                    if (OpenCms.getLog(CmsLog.CHANNEL_MODULE).isWarnEnabled()) {
                        OpenCms.getLog(CmsLog.CHANNEL_MODULE).warn("[CmsProjectDriver] Couldn't find channel " + channelToAdd.get(i) + " errormessage: " + exc.getMessage());
                    }
                }
            }
        } finally {
    ...
    protected void publishOneLine(CmsObject cms, CmsMasterDataSet dataset, int subId, String contentDefinitionName, boolean enableHistory, int versionId, long publishingDate, Vector changedRessources, Vector changedModuleData) throws CmsException {
        ...
        try {
            ...
            OpenCms.getLog(CmsLog.CHANNEL_MODULE).warn("[CmsProjectDriver] error calling method beforePublish in class " + contentDefinitionName);
        }
