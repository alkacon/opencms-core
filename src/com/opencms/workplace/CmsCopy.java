 ...
 * @version $Revision: 1.39 $ $Date: 2001/01/24 09:43:26 $
 */
...
public class CmsCopy extends CmsWorkplaceDefault implements I_CmsWpConstants,I_CmsConstants {
    ...
    private void copyFile(CmsObject cms, CmsFile file, String newFolder, String newFile, 
            String flags) throws CmsException {
        ...
        if((cms.getResourceType(file.getType()).getResourceName()).equals(C_TYPE_PAGE_NAME)) {
            ...
            if(hbodyPath.equals(bodyPath)) {
                ...
                String newbodyPath = (C_CONTENTBODYPATH.substring(0, help)) + newFolder 
                        + newFile;
                    ...
                    try {
                        ...
                        for(int i = 0;i < allFolders.size();i++) {
                            ...
                            if(folder.getState() != C_STATE_DELETED) {
                                cms.copyFolder(folder.getAbsolutePath(), newFolder + newFile 
                                        + "/" + folder.getAbsolutePath().substring(file.getAbsolutePath().length()));
                                checkFlags(cms, newFolder + newFile + "/" 
                                        + folder.getAbsolutePath().substring(file.getAbsolutePath().length()), flags);
                            }
                        }
                        ...
                        for(int i = 0;i < allFiles.size();i++) {
                            ...
                            if(newfile.getState() != C_STATE_DELETED) {
                                copyFile(cms, newfile, newFolder + newFile 
                                        + "/", newfile.getAbsolutePath().substring(file.getAbsolutePath().length()), flags);
                            }
                        }
                        ...
                        try {
                            cms.lockResource(C_CONTENTBODYPATH + (newFolder + newFile 
                                    + "/").substring(1));
                        }
                        catch(CmsException e) {
