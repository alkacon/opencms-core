 ...
 * @version $Revision: 1.1 $ $Date: 2000/02/11 18:48:15 $
...
public class CmsXmlTemplateEditor extends CmsWorkplaceDefault implements I_CmsConstants {
    ...
    public byte[] getContent(A_CmsObject cms, String templateFile, String elementName, Hashtable parameters, String templateSelector) throws CmsException {
        ...
        CmsXmlControlFile controlFile = null;
        ...
        boolean bodychangeRequested = ((exit != null) && "1".equals(bodychange));
        ...
        // OK, the file parameter is checked successfully.
        if(!existsContentParam || templatechangeRequested) {
            // In case of a new editor call or a template change we need the control file.
            controlFile = new CmsXmlControlFile(cms, file);
        }    
        
        if(!existsContentParam) {
            if(controlFile.isElementClassDefined("body")) {
                bodyElementClassName = controlFile.getElementClass("body");
            } 
                
            if(controlFile.isElementTemplateDefined("body")) {
                bodyElementFilename = controlFile.getElementTemplate("body");
            }
            ...
            A_CmsResource tempFile = cms.readFileHeader(bodyElementFilename);
            if(!tempFile.isLocked()) {
                cms.lockResource(bodyElementFilename);
            }
            
            // Now get the currently selected master template file
            layoutTemplateFilename = controlFile.getMasterTemplate();
            layoutTemplateClassName = controlFile.getTemplateClass();
        ...
        if(templatechangeRequested) {
            ...
            controlFile.setMasterTemplate(layoutTemplateFilename);
            controlFile.write();
        }
        ...
        CmsXmlTemplateFile bodyTemplateFile = bodyElementClassObject.getOwnTemplateFile(cms, bodyElementFilename, "body", parameters, null);
        ...
        if(existsContentParam) {
            ...
            try {
                bodyTemplateFile.setEditedTemplateContent(content, templateSelector, oldEdit.equals(C_SELECTBOX_EDITORVIEWS[0]));
            } catch(CmsException e) {
            ...
            System.err.println("*** Writing body template file");
        ...
        session.putValue("te_oldlayout", oldLayoutFilename);       
        session.putValue("te_title", oldTitle);       
