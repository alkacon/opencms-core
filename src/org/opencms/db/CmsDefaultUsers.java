 ...
 * 
 * @version $Revision: 1.8 $ $Date: 2003/09/17 08:31:29 $
 * @since 5.1.5
 */
    ...
    public static CmsDefaultUsers initialize(Configurations conf) throws Exception {
        ...
        try {
            ...
            if (OpenCms.getLog(CmsLog.CHANNEL_MAIN).isErrorEnabled()) {
                OpenCms.getLog(CmsLog.CHANNEL_MAIN).error(". Critical init error/6: " + e.getMessage());
            }
            throw e;
        }
        if (OpenCms.getLog(CmsLog.CHANNEL_INIT).isInfoEnabled()) {            
        ...
        try {
            ...
            if (OpenCms.getLog(CmsLog.CHANNEL_INIT).isInfoEnabled()) {
                OpenCms.getLog(CmsLog.CHANNEL_INIT).info(". Name translation     : non-critical error " + e.getMessage());
            }
        }        
        if (OpenCms.getLog(CmsLog.CHANNEL_INIT).isInfoEnabled()) {            
        ...
        for (int i=0; i<translations.length; i++) {
            ...
            if (valid && ucmatch.startsWith(I_CmsConstants.C_EXPORT_ACEPRINCIPAL_GROUP)) {
                ...
                if (OpenCms.getLog(CmsLog.CHANNEL_INIT).isErrorEnabled()) {
                    OpenCms.getLog(CmsLog.CHANNEL_INIT).error(". Name translation     : ignoring invalid entry '" + match + "'");
                }                 
            }
        }         
    }
