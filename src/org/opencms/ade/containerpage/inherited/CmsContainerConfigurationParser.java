...
public class CmsContainerConfigurationParser {
	...
	public void parse(CmsFile file) throws CmsException {
		...
		for (Locale locale : content.getLocales()) {
			...
			CmsXmlContentRootLocation rootLocation = new CmsXmlContentRootLocation(
					content, locale);
	...
	protected void parseConfigurationGroup(I_CmsXmlContentLocation location) {
		...
		List<I_CmsXmlContentValueLocation> locations = location
				.getSubValues("Configuration");
	 ...
	 */
	protected void parseSingleConfiguration(
		...
		List<I_CmsXmlContentValueLocation> orderKeyLocs = location
				.getSubValues(N_ORDERKEY);
		...
		List<I_CmsXmlContentValueLocation> visibleLocs = location
				.getSubValues(N_VISIBLE);
		...
		List<I_CmsXmlContentValueLocation> hiddenLocs = location
				.getSubValues(N_HIDDEN);
		...
		List<I_CmsXmlContentValueLocation> newElementLocs = location
				.getSubValues(N_NEWELEMENT);
		...
		for (I_CmsXmlContentValueLocation elementLoc : newElementLocs) {
			...
			I_CmsXmlContentValueLocation actualElementLoc = elementLoc
					.getSubValue("Element");
			I_CmsXmlContentValueLocation uriLoc = actualElementLoc
					.getSubValue("Uri");
			CmsUUID structureId = uriLoc.asId(m_cms);
			Map<String, String> settings = CmsXmlContentPropertyHelper
					.readProperties(m_cms, actualElementLoc);
			CmsContainerElementBean newElementBean = new CmsContainerElementBean(
					structureId, CmsUUID.getNullUUID(), settings, false);
			newElementBeans.put(key, newElementBean);
		}
		CmsContainerConfiguration config = new CmsContainerConfiguration(
				ordering, visibilities, newElementBeans);
