...
public class CmsFrameset extends CmsWorkplace {
    ...
    public String buildContextMenues() {
        ...
        for (int i = 0; i < resTypes.size(); i++) {
            ...
            try {
                int resId = getCms().getResourceType(resourceTyp.getResourceName()).getResourceType();
    ...
    public boolean isSyncEnabled() {
        try {
            return null != getCms().getRegistry().getSystemValue(com.opencms.workplace.CmsSyncFolder.C_SYNCHRONISATION_PROJECT);
        } catch (CmsException e) {
            return false;
        }     
    }
