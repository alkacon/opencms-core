 ...
 * @since 6.5.6
 */
public class CmsMirPrSameOtherlockActiveNoPublish implements I_CmsMenuItemRule {
    ...
    public boolean matches(CmsObject cms, CmsResourceUtil[] resourceUtil) {
        ...
        if (resourceUtil[0].isInsideProject()) {
            ...
            return resourceUtil[0].getProjectState() == CmsResourceUtil.STATE_LOCKED_FOR_PUBLISHING
                || !(!lock.isNullLock() && lock.isExclusiveOwnedBy(cms.getRequestContext().currentUser()) && lock.getProjectId() == cms.getRequestContext().currentProject().getId());
        }
