 ...
 * @since 6.5.6
 */
public class CmsMirPrSameLockedActiveChangedAl implements I_CmsMenuItemRule {
    ...
    public CmsMenuItemVisibilityMode getVisibility(CmsObject cms, CmsResourceUtil[] resourceUtil) {
        ...
        char stateAbbr = resourceUtil[0].getStateAbbreviation();
        if (stateAbbr == CmsResourceState.STATE_DELETED.getAbbreviation()) {
            return CmsMenuItemVisibilityMode.VISIBILITY_INVISIBLE;
        } else if (stateAbbr == CmsResourceState.STATE_NEW.getAbbreviation()
            || (resourceUtil[0].getResource().isFile() && stateAbbr == CmsResourceState.STATE_UNCHANGED.getAbbreviation())) {
