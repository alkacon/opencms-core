...
public class CmsResourceTranslator {
    ...
    public CmsResourceTranslator(String[] translations, boolean continueMatching) {
        ...
        for (int i=0; i<m_translations.length; i++) {
            try {
                ...
                if (OpenCms.isLogging(CmsLog.C_OPENCMS_CRITICAL, CmsLog.LEVEL_ERROR)) {
                    OpenCms.log(CmsLog.C_OPENCMS_CRITICAL, CmsLog.LEVEL_ERROR, "["+this.getClass().getName()+"] Malformed resource translation rule:\""+m_translations[i]+"\"");
                }
            }
        }        
        ...
        if (OpenCms.isLogging(CmsLog.C_OPENCMS_DEBUG, CmsLog.LEVEL_ERROR)) {
            OpenCms.log(CmsLog.C_OPENCMS_DEBUG, CmsLog.LEVEL_ERROR, ". Resource translation : " + translations.length + " rules initialized");
        }          
    }    
            ...
            try {
                if (m_perlUtil.substitute(result, m_translations[i], resourceName) != 0) {
                    ...
                    if (OpenCms.isLogging(CmsLog.C_OPENCMS_DEBUG, CmsLog.LEVEL_WARN)) {
                        OpenCms.log(CmsLog.C_OPENCMS_DEBUG, CmsLog.LEVEL_WARN, "["+this.getClass().getName()+"] Resource translation: " + resourceName + " --> " + result);
                    }                    
                    if (DEBUG > 0) {
                ...
                if (OpenCms.isLogging(CmsLog.C_OPENCMS_CRITICAL, CmsLog.LEVEL_WARN)) {
                    OpenCms.log(CmsLog.C_OPENCMS_CRITICAL, CmsLog.LEVEL_WARN, "["+this.getClass().getName()+"] Malformed resource translation rule:\""+m_translations[i]+"\"");
                }
            }
        }
