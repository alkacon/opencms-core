...
public class CmsSynchronize implements I_CmsConstants, I_CmsLogChannels {
     ...
     */
    public CmsSynchronize(CmsObject cms, String resourcePath)
        throws CmsException {
		m_cms = cms;    	
        ...
        if ((m_resourcePath != null) && (m_synchronizePath != null)){
            ...
            + "Error during synchronization, cannot initialize sync process.");
        }
    }
	 ...
	 *
	 * During the synchronisation process, the following actions will be done:<p>
	 * 
	 * <ul>
     ...
     * @throws CmsException if something goes wrong
	 */
	private void syncVfsFs(String folder) 
		throws CmsException {	
		int action=0;
		...
		for (int i=0;i<resources.size();i++){	
			...
			if (res.getState()!=C_STATE_DELETED) {		
				...
				if (res.isFolder()){
					...
					// do the correct action according to the test result				
					switch (action) {
					case C_EXPORT_VFS:	exportToFS(res);			
										break;
									
					case C_UPDATE_VFS:	updateFromFs(res);				
										break;		
									
					case C_DELETE_VFS:	deleteFromVfs(res);			
										break;							
				
					default:			skipResource(res);
	...
	private void removeFromFs(String folder)throws CmsException {
		...
		for (int i=0;i<res.length;i++){
			...
			CmsSynchronizeList sync=
				(CmsSynchronizeList)m_syncList.get(translate(vfsFile));
     ...
     * @throws CmsException if something goes wrong
	 */	
	private void copyFromFs(String folder)
		throws CmsException {
		// get the corresponding folder in the FS
		File[] res;			
		...
		try {
			...
			CmsResource newFolder=m_cms.createResource(translate(folder), 
								  C_TYPE_FOLDER_NAME, new HashMap(),
							      new byte[0], null);
			...
			CmsSynchronizeList sync=new CmsSynchronizeList(folder,resourcename,
		...
		if (fsFile.isDirectory()){
			...
			for (int i=0;i<res.length;i++){
				...
				resname=resname.substring(m_synchronizePath.length(),
								resname.length());
	 ...
	 * @throws CmsException if something goes wrong
	 */
	private void importToVfs(File fsFile,String resName,String folder) 
	throws CmsException {
		try {
		    ...
		    CmsFile newFile = (CmsFile)m_cms.createResource(translate(folder) , 
										filename, type, null, content);
			// now check if there is some external method to be called which
			// should modify the imported resource in the VFS
			Iterator i = m_checkFile.iterator();
			while (i.hasNext()) {
			...
			CmsSynchronizeList syncList=new CmsSynchronizeList(resName,
											translate(resName),
	...
	private void exportToFS(CmsResource res) throws CmsException {	
		...
		try {
			...
			if (res.getState()!=C_STATE_DELETED){
				...
				CmsSynchronizeList syncList=new CmsSynchronizeList(resourcename,
												translate(resourcename),res.getDateLastModified(),
												fsFile.lastModified());
	...
	private void updateFromFs(CmsResource res) throws CmsException {	
		...
		try {
			...
			//add resource to synchronisation list
			CmsSynchronizeList syncList=new CmsSynchronizeList(sync.getResName(),
				...
				while (line!=null){
					...
					if (line!= null) {
						...
						if (tok!= null){
							...
							CmsSynchronizeList sync=new CmsSynchronizeList(resName,
														tranResName,
														modifiedVfs,modifiedFs);
    ...
    private void createNewLocalFile(File newFile) throws CmsException {
        ...
        if (newFile.exists()){
            throw new CmsException("["+this.getClass().getName()
            			+ "] "+newFile.getPath()+" already exists on filesystem");
        }
        try {
            File parentFolder = new File(newFile.getPath().replace('/', 
				File.separatorChar).substring(0,newFile.getPath().lastIndexOf(File.separator)));
            parentFolder.mkdirs();
            if (parentFolder.exists()){
                fOut = new FileOutputStream(newFile);
            } else {
                throw new CmsException("["+this.getClass().getName()
                			+ "]"+" Cannot create directories for "+newFile.getPath());
            }
        } catch (IOException e){
            throw new CmsException("["+this.getClass().getName()
            		+ "]"+" Cannot create file "+newFile.getPath()+" on filesystem", e);
        } finally {
            if (fOut != null){
                try {
                    fOut.close();
