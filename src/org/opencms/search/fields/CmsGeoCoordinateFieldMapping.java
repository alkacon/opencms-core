...
public class CmsGeoCoordinateFieldMapping implements I_CmsSearchFieldMapping {
            ...
            try {
                switch (entry.getType()) {
                    case field:
                        ...
                        String coord = parseCoordinates(value);
                        if (coord != null) {
                            return coord;
                        }
                        break;
    ...
    /**
     * Parses coordinates, either from lat,lon pair or from the JSON generated by the location picker.
     *
     * @param coord the coordinate string
     * @return the parsed coordinates
     */
    private String parseCoordinates(String coord) {

        if (coord == null) {
            return null;
        }
        String locationPickerCoordinates = CmsGeoCoordinateUtil.parseLocationPickerCoordinates(coord);
        if (locationPickerCoordinates != null) {
            return locationPickerCoordinates;
        } else if (CmsGeoCoordinateUtil.validateCoordinates(coord)) {
            return coord;
        }
        return null;
    }
