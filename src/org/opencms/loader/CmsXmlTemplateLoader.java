...
public class CmsXmlTemplateLoader implements I_CmsResourceLoader {
    ...
    protected byte[] generateOutput(CmsObject cms, CmsFile file, I_CmsRequest req) throws CmsException {
                ...
                while (parameters.hasMoreElements()) {
                    ...
                    if (paramValue != null) {
                        ...
                        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO)) {
                            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, getClassName() + "Empty parameter \"" + paramName + "\" found.");
                        }
                    }
                }
            }
        ...
        while (urlParameterNames.hasMoreElements()) {
            ...
            if (paramValue != null) {
                ...
                if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO)) {
                    OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, getClassName() + "Empty URL parameter \"" + pname + "\" found.");
                }
            }
        }
            ...
            try {
                ...
                if (!(tmpl instanceof I_CmsXmlTemplate)) {
                    ...
                    if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR)) {
                        OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR, getClassName() + errorMessage);
                    } 
                ...
                if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO)) {
                    OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsXmlLoader] There were exceptions while generating output for " + cms.readAbsolutePath(file));
                    OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_INFO, "[CmsXmlLoader] Clearing template file cache for this file.");
                }
    ...
    private I_CmsTemplate getTemplateClass(String classname) throws CmsException {
        if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_DEBUG)) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_DEBUG, getClassName() + "Getting start template class " + classname + ". ");
        }
        ...
        if (!(o instanceof I_CmsTemplate)) {
            ...
            if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR)) {
                OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR, "[CmsTemplateClassManager] " + errorMessage);
            }
    ...
    public void init(Configurations conf) {
        ...
        if (OpenCms.isLogging(CmsLog.CHANNEL_INIT, CmsLog.LEVEL_INFO))
            OpenCms.log(CmsLog.CHANNEL_INIT, CmsLog.LEVEL_INFO, ". Element cache        : " + (enableElementCache ? "enabled" : "disabled"));
        if (enableElementCache) {
            try {
                ...
                if (OpenCms.isLogging(CmsLog.CHANNEL_INIT, CmsLog.LEVEL_INFO))
                    OpenCms.log(CmsLog.CHANNEL_INIT, CmsLog.LEVEL_INFO, ". Element cache        : non-critical error " + e.toString());
            }
    ...
    private void processXmlTemplate(CmsObject cms, CmsFile file) throws CmsException {
        ...
        if ((DEBUG > 0) && OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR)) {
            OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_ERROR, getClassName() + "Loader started for " + file.getName());
        }
    ...
    private void writeBytesToResponse(CmsObject cms, byte[] result) throws CmsException {
        try {
            ...
            if (OpenCms.isLogging(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_DEBUG)) {
                OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_DEBUG, getClassName() + "IO error while writing to response stream for " + cms.getRequestContext().getFileUri());
                OpenCms.log(CmsLog.CHANNEL_MAIN, CmsLog.LEVEL_DEBUG, getClassName() + ioe);
            }
        } catch (Exception e) {
