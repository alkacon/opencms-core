...
public final class CmsContentEditor extends CmsEditorBase {
    ...
    /** The help button. */
    private CmsPushButton m_helpButton;
    ...
    private void initToolbar() {
        ...
        m_helpButton = createButton(
            org.opencms.gwt.client.Messages.get().key(org.opencms.gwt.client.Messages.GUI_RESOURCE_HELP_0),
            I_CmsToolbarButtonLayoutBundle.INSTANCE.toolbarButtonCss().toolbarResourceHelp());
        m_helpButton.addClickHandler(new ClickHandler() {

            public void onClick(ClickEvent event) {

                CmsResourceHelpDialog helpDialog = new CmsResourceHelpDialog(null, CmsResourceHelpDialogType.EDITOR);
                helpDialog.loadAndShow();
            }
        });
        m_toolbar.addRight(m_helpButton);
        if (CmsCoreProvider.get().isResourceHelpDialogActive(CmsResourceHelpDialogType.EDITOR)) {
