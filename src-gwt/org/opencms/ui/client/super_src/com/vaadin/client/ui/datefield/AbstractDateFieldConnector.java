...
public abstract class AbstractDateFieldConnector<R extends Enum<R>>
    ...
    private Map<R, Integer> getTimeValues() {
        ...
        // PATCH for Collectors.toMap behaving differently in GWT 2.13
        Map<R, Integer> result = new java.util.HashMap<>();
        resolutions.forEach(res -> {
            if (resolution.compareTo(res) <= 0) {
                result.put(res,stateResolutions.get(res.name()));
            } else {
                result.put(res, null);
            }
        });
    ...
    protected Map<R, Integer> getDefaultValues() {
        ...
        resolutions.forEach(res -> {
            if (resolution.compareTo(res) <= 0) {
                result.put(res,stateResolutions.get("default-" + res.name()));
            } else {
                result.put(res, null);
            }
        });
