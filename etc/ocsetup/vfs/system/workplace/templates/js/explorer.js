...
function folderOpen(id){
        ...
        while(id!=tree.root.id){
        var nodeName='_n'+id;
            if(tree.nodes[nodeName].childs.length>0){
                tree.nodes[nodeName].open=true;
            }
            id = tree.nodes[nodeName].parent.id;
...
//  doc.writeln("<a href='javascript:top.openFolder(&quot;"+ node.id +"&quot;);' target='explorer_files'  class='treefolder' ;> &nbsp;"+ node.name + "</a></td></tr><tr valign=bottom><td valign=bottom align=left nowrap>");
    ...
    if(node.isGrey)var foco="class=isGrey ";
        else var foco="class=treefolder ";
...
function showTree(doc) {
    ...
    var showTreeHead="<html><head><title>opencms explorer tree</title>"+
            ...
            "a.isGrey { text-decoration: none; color: #8c8c8c; font-family:arial; font-size:8pt;}"+
            "a.treefolder { text-decoration: none; color: #000000; font-family:MS Sans Serif, Arial, helvetica, sans-serif; font-size:8px;}"+
...
function displayHead(doc){
    ...
    var headHead="<html><head><title>opencms</title>"+
            ...
            "<form name=urlform onSubmit='javascript:top.addHist(top.vr.actDirectory);top.openurl();return false;'>"+
    ...
    var headFoot="<td class=menu width=30px nowrap align=right>&nbsp;</td>"+
            ...
            "<td class=menubold nowrap align=right valign=middle><p class=einzug><font face=arial size=2> <b>&nbsp;"+vr.langadress+"</b>&nbsp; </td>"+
                    ...
                    if(vi.menus[vi.liste[i].type].items[a].rules.charAt(0)=='0'){
						...
						if(vi.menus[vi.liste[i].type].items[a].rules.charAt(0)=='1'){
							...
							if(vi.menus[vi.liste[i].type].items[a].rules.charAt(0)=='2'){
								...
								if(vi.liste[i].type==0) wo.writeln("<TR><TD><A class=kontextlink href='"+vi.menus[vi.liste[i].type].items[a].link+"&lasturl="+returnplace+"&file="+vr.actDirectory+vi.liste[i].name+"/'>"+vi.menus[vi.liste[i].type].items[a].name+"</a></td></tr>");
								 else wo.writeln("<TR><TD><A class=kontextlink href='"+vi.menus[vi.liste[i].type].items[a].link+"&lasturl="+returnplace+"&file="+vr.actDirectory+vi.liste[i].name+"' target="+vi.menus[vi.liste[i].type].items[a].target+">"+vi.menus[vi.liste[i].type].items[a].name+"</a></td></tr>");
								}
							}
						}
                    }
 ...
 */
function dU(doc){
	if(vi.liste.length >20){
		doc.open();
		doc.writeln("<html><body><center><br><br><br><br><font face=Helvetica size=2>Building Page...</center></body></html>");
		doc.close();
	}
	vi.lastLayer=null;
    vi.locklength=0;
